...
Project : Hydraulic System Control
Version : 1.0
Date    : 10/26/2022
...
char S7=0;
char S8=0;
//char cyl=0; //used for polling to send to lcd -1 is retracting, 0 is home, 1 is extending, 2 destination
//char sol=0;
char PB1, PB2;
char e_Saw, e_Clamp, r_Saw, r_Clamp;
...
//Read Switches
e_Clamp = ~PINA & 0b10000000;		//C1 extended
r_Clamp = ~PINA & 0b01000000;		//C1 retracted
e_Saw = ~PINA & 0b00100000;	//C2 extended
r_Saw = ~PINA & 0b00010000;	//C2 retracted
...
// add flow rate (step 15)
//state machine Equations
S0 =(S0||(S7 && (!PB1 && !PB2))) && !S1;
S1 =(S1||(S0 && PB1)) && !S2 && !S6;
S2 =(S2||(S1 && e_Clamp && PB1)) && !S3 && !S6;
S3 =(S3||(S2 && PB1 && PB2)) && !S8 && !S4;
S4 =(S4||(S3 && (!PB1||!PB2))) && !S5;
S5 =(S5||(S4 && r_Saw)) && !S6;
S6 =(S6||(S5 && timeReached)||(S1&&!PB1)||(S2&&!PB1)) && !S7;
S7 =(S7||(S6 && r_Clamp)) && !S0;
S8 =(S8||(S3 && (PB1 && PB2 && e_Saw))) && !S6;
if(S0 && !S1 && !S2 && !S3 && !S4 && !S5 && !S6 && !S7 && !S8 ){ //ready
	while(r_Saw != 0b00010000){
		PORTA = (~PINA & 0x0F) | 0x01; //retract saw
	}
	while(r_Clamp != 0b01000000){
		PORTA = (~PINA & 0x0F) | 0x04; //retract clamp
	}
	lcd_putsf("Retract all");
	
}

if(!S0 && S1 && !S2 && !S3 && !S4 && !S5 && !S6 && !S7 && !S8 ){ //extending clamp
	PORTA = (~PINA & 0x0F) | 0x08;
	flowRateCount++;
	lcd_putsf("extending clamp");
	}
if((S6||S8)&&!S0 && !S1 && !S2 && !S3 && !S4 && !S5 && !S7){
	PORTA = (~PINA & 0x0F) | 0x04;
	lcd_putsf("retracting clamp");
}
if((S4||S8)&&!S0 && S1 && !S2 && !S3 &&!S5 && !S6 && !S7){
	PORTA = (~PINA & 0x0F) | 0x02;
	lcd_putsf("retracting saw");
}
if((S3)&&!S0 && !S1 && !S2 && !S4 && !S5 && !S6 && !S7 && !S8){
	PORTA = (~PINA & 0x0F) | 0x01;
	lcd_putsf("extending saw");
}


//Output
if(S0 && !S1 && !S2 && !S3 && !S4 && !S5 && !S6 && !S7 && !S8 ){ //ready
	while(e_Saw != 0b00010000){
		PORTA = (~PINA & 0x0F) | 0x01; //retract saw
	}
	while(r_Clamp != 0b01000000){
		PORTA = (~PINA & 0x0F) | 0x08; //retract clamp
	}
	lcd_putsf("Retract all");
...
if(!S0 && !S1 && S2 && !S3 && !S4 && !S5 && !S6 && !S7 && !S8 ){ //clamp armed and ready saw
	lcd_putsf("Clamp extended");
	flowRate = flowRateCount;
	flowRateCount = 0;
	}

if(!S0 && !S1 && !S2 && S3 && !S4 && !S5 && !S6 && !S7 && !S8 ){//saw extending
	PORTA = (~PINA & 0x0F) | 0x02;
	lcd_putsf("extending saw");
	}

if(!S0 && !S1 && !S2 && !S3 && S4 && !S5 && !S6 && !S7 && !S8 ){//shutdown mode
	PORTA = (~PINA & 0x0F) | 0x01; //retract saw
	lcd_putsf("state4");
	}

if(!S0 && !S1 && !S2 && !S3 && !S4 && S5 && !S6 && !S7 && !S8 ){//wait mode
delay_us(5000000);
timeReached=true;
	lcd_putsf("state5");
	}

if(!S0 && !S1 && !S2 && !S3 && !S4 && !S5 && S6 && !S7 && !S8 ){ //retrcting cylinder
	PORTA = (~PINA & 0x0F) | 0x08;
	lcd_putsf("state 6");
	}

if(!S0 && !S1 && !S2 && !S3 && !S4 && !S5 && !S6 && S7 && !S8 ){//safety
	while(PB1||PB2){
	}
	lcd_putsf("state 7");
	}

if(!S0 && !S1 && !S2 && !S3 && !S4 && !S5 && !S6 && !S7 && S8 ){//retracting saw
	PORTA = (~PINA & 0x0F) | 0x01;
	lcd_putsf("state 8");
	}

//display where clamp and saw are at
if((e_Saw & 0b00100000)==0b00100000){
	lcd_puts("extended_saw");
}
if((r_Saw & 0b00010000)==0b00010000){
	lcd_puts("home_saw");
}
if((e_Clamp & 0b10000000)==0b10000000){
	lcd_puts("extended_clamp");
}
if((r_Clamp & 0b01000000)==0b01000000){
	lcd_puts("home clamp");
}
timeReached=false;
