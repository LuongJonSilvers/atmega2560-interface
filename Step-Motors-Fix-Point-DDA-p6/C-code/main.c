...
Project : DDA Stepper Motor Technique
Version : 1.0
Date    : 10/12/2022
Author  : Viral Yemul & Jonathan Luong
Company : ME 445 - Mechatronics in Control
Comments: 
...
int position_accumulator = 0;
int velocity_accumulator = 0;
int acceleration = 5;
bool kickoff_play = false;
...
/**PORTA=~PORTA;
delay_us(20);				//understanding delay part, not a question answer
PORTA=~PORTA;
**/

uint8_t temp;				//question 8, produces pulses with a length of 2microseconds and does it every 8e-4 seconds
//temp = PORTA;
//PORTA=((~temp)&0b00000001);
//delay_us(2);
//PORTA=temp;
...
velocity_accumulator = velocity_accumulator + acceleration;


if(velocity_accumulator>=17477){
		velocity_accumulator=17476;
		}
else if(velocity_accumulator<=-17477){
	velocity_accumulator=-17476;
}


if(velocity_accumulator < 0){
	PORTA = PORTA | 0b00000010; //cw
	}
	else{
		PORTA = PORTA & 0b11111101; //0 ccw
	}
position_accumulator = position_accumulator + velocity_accumulator;

if((SREG & 0b00001000) == 0b00001000){		//overflow happened
	temp = PORTA;
	PORTA=((temp)|0b00000001);
	delay_us(3);
	PORTA=temp;
...
//reset overflow flag
	position_accumulator =  position_accumulator - 1;
//subtract position accumulator back to what it should be
//send pulse (make send pulse function)
}
}
void main(void)
{
...
while (1)
	{
      ...
      // Place your code here
	if((~PINL & 0b00000001) == 0b00000001){
			acceleration = -5; 
      }
	  else if((~PINL & 0b00000010) == 0b00000010){
			acceleration = 5;
	  }
	  else if((~PINL& 0b00000100) == 0b00000100){
		  acceleration = 0 ;
		  velocity_accumulator = 0;
		  position_accumulator = 0; 
	  }
	  else if((~PINL& 0b00001000) == 0b00001000){
		  kickoff_play = true;
		  while(time_accumulator<3125){
		  acceleration = 5;
		  }
		  while((time_accumulator>=3125)&&(time_accumulator<=9375)){
		  acceleration = 0;
		  }
		  while((time_accumulator>=9376)&&(time_accumulator<=12500)){
		  acceleration = -5;
		  }
		  if(time_accumulator>=12500){
		  kickoff_play =false;
		  time_accumulator = 0;
		  }
	  }
