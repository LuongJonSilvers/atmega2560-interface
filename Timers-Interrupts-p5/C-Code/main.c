...
Project : Timers, Interrupts, & Data Structures
Version : 1.0
Date    : 10/5/2022
Author  : Viral Yemul & Jonathan Luong
Company : Mechatronics Lab - Univ. of  Wisconsin, Madison
Comments: 
...
struct twobytes			// Q8 - Creating data structure to allow bite access to an unsigned integer.
{
	char lsb;
	char msb;
	}bytes;
...
union byteint position;

struct ourLong			// Q10 - Creating data structure to allow byte access to long interger value.
{
	union byteint first;
	union byteint second;	
	};

union wholeLong
{
	struct ourLong intbyte;
	long longval;
	};
union wholeLong positionLong;	

union wholeFloat		// Q12 - Creating data structures to overlay an array.  
{
	struct ourLong floatbyte;
	float floatval;
	};
union wholeFloat positionFloat;	
...
PORTA = ~PINA;			//Q4 - To verify the timer0 interrpt. 
}
...
{
// Place your code here
PORTL = ~PINL;			//Q5 - To verify the timer1 interrupt.
}
...
DDRA=(0<<DDA7) | (0<<DDA6) | (0<<DDA5) | (0<<DDA4) | (0<<DDA3) | (0<<DDA2) | (0<<DDA1) | (1<<DDA0);						// Modify for Q4 for timer0 interrupt  
...
// Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=In Bit2=In Bit1=In Bit0=In 
DDRL=(0<<DDL7) | (0<<DDL6) | (0<<DDL5) | (0<<DDL4) | (0<<DDL3) | (0<<DDL2) | (0<<DDL1) | (1<<DDL0);			// Modify for Q6 for timer0 interrupt
// State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=T Bit2=T Bit1=T Bit0=T 
...
TIMSK0=(0<<OCIE0B) | (0<<OCIE0A) | (1<<TOIE0);		//Modify for Q3 - to enable timer0

// Timer/Counter 1 Interrupt(s) initialization
TIMSK1=(0<<ICIE1) | (0<<OCIE1C) | (0<<OCIE1B) | (1<<OCIE1A) | (0<<TOIE1);	//Modify for Q5 - to enable timer1
...
while (1)
      {
		  // Place your code here
		  // data structure results: Q12, Q10, Q9. 
		  position.bytes.msb=0x89;			
		  position.bytes.lsb=0x90;
		  temp1 = position.bytes.msb;
		  temp2 = position.bytes.lsb;
      		
		positionLong.longval = 0x12345678;
		positionFloat.floatval = 3.14159;		
